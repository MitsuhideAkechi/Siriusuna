...
We can use [identity operators](Mutable_Variable#^ggr4iq.md) to test if two expression evaluate to same object.
...
3. when it has a [base class](Inheritance#^cy20w2.md), looking up its base classes recursively.
...
- Instance Attribute Assignment:  
	...
	Then when we look up for this attribute, the one in instance will be returned and will not changes in line with class's.([Remember order.](Object-Oriented_Programming(OOP)#^ngtggr.md))
